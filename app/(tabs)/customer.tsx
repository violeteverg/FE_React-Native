...
export default function CustomerPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["CUSTOMER", page, searchQuery],
    queryFn: () => getAllCustomer(page, searchQuery),
  });

  const handleUpdate = (id: string) => {
    setCustomerId(id);
    setType("update");
    router.push(`/form/customer?mode=update&id=${id}`);
  };

  const handleDelete = (id: string) => {
    router.push({
      pathname: "/remove/customer/[id]",
      params: { id },
    });
  };
  ...
  const handleCreate = () => {
    setType("create");
    router.push("/form/customer?mode=create");
  };
